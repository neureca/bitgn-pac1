...
- `StepResultKind in {observed_facts, predicate_snapshot, obligation_patch, blocker_patch, planned_action, execution_result, validation_result, adapter_result}`
...
StepResult evaluate_predicates(AgentsState state, RuntimeState runtime, ObservedFacts facts)

StepResult derive_obligations(AgentsState state, PredicateSet predicates)

StepResult derive_blockers(AgentsState state, PredicateSet predicates)
...
while runtime_state.ended != true:
    ...
    predicates = evaluate_predicates(agent_state, runtime_state, observed.payload)
    agent_state, runtime_state = apply_step_result(agent_state, runtime_state, predicates)

    obligation_patch = derive_obligations(agent_state, predicates.payload)
    agent_state, runtime_state = apply_step_result(agent_state, runtime_state, obligation_patch)

    blocker_patch = derive_blockers(agent_state, predicates.payload)
    agent_state, runtime_state = apply_step_result(agent_state, runtime_state, blocker_patch)
